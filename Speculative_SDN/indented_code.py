...
BATCH_SIZE = 32  # number of data samples propagated through the network before parameters are updated
LR = float(args.LR / 10)  # learning rate
...
MEMORY_CAPACITY = args.memory_capacity  # size of experience replay buffer
...
class Net(nn.Module):
    def __init__(self, N_STATES, N_ACTIONS):
        ...
        self.fc1 = nn.Linear(N_STATES, 10)  # the linear layer
...
def cal_reward(X_selected, controllerTable, oldControllerTable, step, w):
    ...
    for i in range(len(controller)):
        # reward aging factor
        controllerTable.iloc[i, 4] = (float(args.rewardAgingFactor / 10)) * controllerTable.iloc[i, 4]
        ...
        if controllerTable.iloc[controllernumber, 5] != 0:
            if oldControllerTable.iloc[controllernumber, 5] < controllerTable.iloc[controllernumber, 5]:
                ...
                while j in range(0, math.ceil(w / 2)):
                    ...
                    # 'number' is not defined in the snippet,
                    # presumably it should be 'controllernumber' or some other variable
                    # but I'll leave your logic as is.
                    if ((number - j) >= 0):  # 'number' not declared in snippet
                        reward[number - j] += spatialReward

                    j += 1
                    spatialReward *= (float(args.spatialReward / 10))
...
action_list = np.random.randint(
    N_ACTIONS,
    size=math.ceil(N_flows / numberofFlowsPerAgent)
)
...
while True:
    ...
    while True:
        if reactive is True:
            ...
            if len(queue) != 0 and queue_ is False:
                tempqueue = queue[0]
                queue_ = True
            ...
            if processFlow is True:
                # check if the switch table is full, remove the least frequently used flow rule
                if tablesize == len(switch):
                    ...
                    for k in range(len(switch)):
                        ...
                        if reactive is True and speculative is False:
                        ...
                        if reactive is True and speculative is True:
            ...
            # if the entry is in the selected set, increment hits
            ...
            if found is True:
                for j in range(len(controller)):
                    if (controller.iloc[j, 0] == datasetcopy.iloc[0, 0] and
                       controller.iloc[j, 1] == datasetcopy.iloc[0, 1]):
        ...
        # If more than 0.9 seconds, reset the counter for LFU
        if ((value.loc[new] - value.loc[least]) > LFUTimeInterval).bool():
            least = new
            controller['counter'] = 0

        # store total hits and misses per second for the graph
    ...
    if speculative is True:
        ...
        # the agents select the flow rules to include in the switch table
        ...
        for i in range(len(switch)):
            for j in range(len(X_selected)):
                if (X_selected.iloc[j, 1] == switch.iloc[i, 0] and
                   X_selected.iloc[j, 2] == switch.iloc[i, 1]):
        ...
        # if the selected set is < table size, copy the table
        # start a loop with size of selected flow rules to install flow rules
        # check if switch table is full to remove LFU
        ...
        w = 2 * (math.ceil(len(controller) / flowresult))  # 'flowresult' not defined above
        ...
        switch = switch[
            switch.groupby(['Source', 'Destination'])['age'].transform('max') == switch['age']
        ]
...
if speculative is True:
    ...
    result, = plt.plot(y, x)
    getxdata = list(result.get_xdata())
    getxdata.insert(0, "seconds")
    getxdata = [str(i) for i in getxdata]

    getydata = list(result.get_ydata())

...
class SpeculativeSimulation:
    ...
    def run(self, dataset, value):
        """Main simulation loop for speculative mode"""
        ...
        while True:
            ...
            if self.logger:
                self.logger.lti_info(self.data_collector.current_lti, 
                                   ...
                                   f"Speculative flows: {sum(1 for _, flow in self.switch_table.iterrows() if 'is_speculative' in flow and flow['is_speculative']) if 'is_speculative' in self.switch_table.columns else len(self.switch_table)}, "
    ...
    def _process_single_packet(self, dataset, value):
        """Process a single packet (same as reactive but without queue processing)"""
        # Create packet data
        ...
        if packet_found:
            ...
            self._handle_packet_miss(packet_data)
            # Record miss
    ...
    def _check_packet_match(self, packet_data):
        ...
        for j in range(len(self.switch_table)):
            if (self.switch_table.iloc[j]['Source'] == packet_data.iloc[0]['Source'] and 
                ...
                # In speculative mode, all flows are speculative
                is_reactive_hit = False
    ...
    def _handle_packet_miss(self, packet_data):
        """Handle packet miss"""
        for j in range(len(self.controller_table)):
            if (self.controller_table.iloc[j]['Source'] == packet_data.iloc[0]['Source'] and 
                self.controller_table.iloc[j]['Destination'] == packet_data.iloc[0]['Destination']):
    ...
    def _perform_speculative_learning(self, dataset, value):
        """Perform speculative learning and flow installation"""
        # Store old controller state for reward calculation
        old_controller_table = self.controller_table.copy()
        ...
        if evictable_flows > 0 or available_slots > 0:
            # There are flows that can be evicted, proceed with speculative flow installation
        ...
        reward_list = self.reward_function.calculate_rewards(
            selected_flows, self.controller_table, old_controller_table, 
    ...
    def _select_flows_based_on_actions(self, agent_actions):
        ...
        flow_selection['Source'] = self.controller_table['Source']
        flow_selection['Destination'] = self.controller_table['Destination']

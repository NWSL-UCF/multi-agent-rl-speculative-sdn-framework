#!/usr/bin/env python3
"""Plot best speculation efficiency vs aging factor, one panel per algorithm."""
...
def read_metric(summary_path: Path) -> float | None:
...
def load_best_series_for_algo(
    ...
    """Best speculation efficiency per aging factor and the ordering that achieved it."""
    by_af: dict[float, tuple[float, str]] = {}
    base = root / f"mode_{MODE}" / f"algorithm_{algorithm}"
    for summary_path in base.rglob(f"tablesize_{tablesize}/agingfactor_*/summary.json"):
        ...
        value = read_metric(summary_path)
        if value is None:
            continue
        prev = by_af.get(af, (float("-inf"), ordering))[0]
        if value > prev:
            by_af[af] = (value, ordering)
    return sorted((af, value, ordering) for af, (value, ordering) in by_af.items())
...
def compute_shared_ylim(root: Path) -> tuple[float, float]:
    ...
    for algorithm in ALGORITHMS:
        for tablesize in TABLESIZES:
            series = [
                (af, value, ordering)
                for af, value, ordering in load_best_series_for_algo(root, algorithm, tablesize)
                if af in AGING_FACTORS
            ]
            values.extend(value for _, value, _ in series)
    if not values:
        return 0, 1
...
def draw_algo_panel(
    ...
    *,
    ...
    for tablesize in TABLESIZES:
        ...
        ys = [value for _, value, _ in series]
    ...
    if show_ylabel:
        ax.set_ylabel("Speculation Efficiency")
...
def make_legend(fig: plt.Figure) -> None:
    ...
    add_boxed_legend(fig, mode_handles, 0.81, y_top, ncol=1, caption="Mode")
...
def plot_by_algorithm(root: Path, output_dir: Path) -> tuple[Path, Path]:
    ...
    base = output_dir / f"{MODE}_best_speculation_efficiency_by_algorithm"
...
def main() -> None:
    ...
    ylim = compute_shared_ylim(root)
    print(f"shared y-axis {ylim[0]:.2f}–{ylim[1]:.2f}")

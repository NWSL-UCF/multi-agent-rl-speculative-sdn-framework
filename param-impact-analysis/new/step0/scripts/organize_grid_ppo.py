#!/usr/bin/env python3
"""Reorganize flat PPO job folders into a hierarchical tree under results/grid/ppo.

Hierarchy:
  mode_{mode}/objective_{objective}/gamma_{gamma}/ppo_lr_{lr}/
  rewardAgingFactor_{raf}/spatialReward_{sr}/hidden_layers_{layers}/
  ppo_epochs_{epochs}/ordering_{ordering}/trace_{trace}/seed_{seed}/

``agingfactor`` is not part of the path (it is determined by ``objective``).
When multiple flat folders map to the same path (duplicate runs), the first
copy is kept and later ones are skipped.
"""
...
DEFAULT_SRC = STEP0_DIR.parents[2] / "results" / "grid" / "algorithm_ppo"
DEFAULT_DST = STEP0_DIR.parents[2] / "results" / "grid" / "ppo"

#!/usr/bin/env python3
"""Reorganize flat bandit job folders into a hierarchical tree under results/grid/bandit.

Hierarchy:
  mode_{mode}/objective_{objective}/bandit_c_{c}/rewardAgingFactor_{raf}/
  spatialReward_{sr}/ordering_{ordering}/trace_{trace}/seed_{seed}/
...
DEFAULT_SRC = STEP0_DIR.parents[2] / "results" / "grid" / "algorithm_bandit"
DEFAULT_DST = STEP0_DIR.parents[2] / "results" / "grid" / "bandit"
...
def dest_dir_for_args(args: dict, dst_root: Path) -> Path:
    parts = [
        ...
        f"bandit_c_{_fmt(args['bandit_c'])}",
